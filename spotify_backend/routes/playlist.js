...
//colon means that this playlistId act as a variable now.....now if anyone call /get/playlist/yhsdgsdjhfbs the playlistId variable gets assigned the value
router.get(
...
router.get(
  ...
  async (req, res) => {
    ...
    const artist = await User.findOne({ _id: artistId }).exec();
    if (!artist) {
      return res.status(304).json({ err: "Invalid Artist Id" });
    }
    const playlists = await Playlist.find({ owner: artistId }).exec();
